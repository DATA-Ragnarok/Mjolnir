        ...
        >
          {stories.map(story => (
            <SortableUserStoryCard 
              ...
              wipWarning={
                (story.status === 'In Progress' && !!story.assignedUser && (userInProgressCount[story.assignedUser._id || story.assignedUser.id || ''] || 0) > 1) ||
                (story.status === 'Waiting for MR' && !!story.assignedUser && (userWaitingMRCount[story.assignedUser._id || story.assignedUser.id || ''] || 0) > 1)
              }
            />
          ))}
        </div>
